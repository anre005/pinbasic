...
that any larger value bigger gets the exponential function to overflow and return
infinite results.
...
Hence, $\log\left(\exp(x + y - m) + \exp(z - m)\right)$ with $m = \max(x + y, z) = 900$
is a more stable and accurate expression.
Irrelevant of the specified values for $x,y$ and $z$, one of the terms $x + y - m$ and $z - m$
is always zero, whereas % depending on the magnitude of the terms.
the remaining term is always less than 0.
This yields to small input values for the exponential function which in turn is always bigger than 1.
...
$$
\begin{align}
  & \log\left(\exp(x + y - \max(x + y, z)) + \exp(z - m)\right) = \notag \\
  & \log\left(\exp(800 - 400 - \max(400, 900)) + \exp(900 - 900)\right) = \notag \\
  & \log\left(\exp(-500) + 1 \right) = `r log1p(exp(400 - 900))` \notag
\end{align}
$$
