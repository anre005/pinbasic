...
\prsym\left(B\right) &= \left(1-\probinfevent\right)\left(\exp\left(-\intensuninfbuys \right)\dfrac{\intensuninfbuys^{B_{\daysym}}}{B_{\daysym}!} \right) \notag \\
                     & + \probinfevent \left(1-\probbadnews\right) \left(\exp\left(-(\intensuninfbuys + \intensinf) \right)\dfrac{\left(\intensuninfbuys + \intensinf
                     \right)^{B_{\daysym}}}{B_{\daysym}!} \right) \notag \\
                     & + \probinfevent \probbadnews \left(\exp\left(-\intensuninfbuys \right)\dfrac{\intensuninfbuys^{B_{\daysym}}}{B_{\daysym}!} \right)
\end{align}
$$
and
$$
...
\prsym\left(S\right) &= \left(1-\probinfevent\right)\left(\exp\left(-\intensuninfsells \right)
                      \dfrac{\intensuninfsells^{S_{\daysym}}}{S_{\daysym}!} \right) \notag \\
                     ...
                     \dfrac{\left(\intensuninfsells + \intensinf \right)^{S_{\daysym}}}{S_{\daysym}!} \right).
\end{align}
$$
It is obvious that both marginal densities
...
\end{align}
$$
The equations for the expected values
...
Starting values for $\probinfevent$ and $\probbadnews$ are limited to be a series of equidistant
...
In principle, any value reasonably bigger than 0 can be chosen as lower bound and
...
Howeber, not all combinations are feasible due to negative values for
